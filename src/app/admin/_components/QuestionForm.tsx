...
export default function QuestionForm({ subjects, topics }: Props) {
  ...
  function updateOption(i: number, patch: Partial<OptionState>) {
    setOptions(prev => prev.map((opt, idx) => (idx === i ? { ...opt, ...patch } : opt)))
  }
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      // 1. Upload main question image if a file was selected
      ...
      // 2. Upload option images (sequentially to surface errors clearly)
    ...
    <div className="min-h-screen bg-gray-950 py-12 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-2xl font-bold text-white">Añadir pregunta</h1>
        </div>
